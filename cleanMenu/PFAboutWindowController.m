...
- (void)windowDidLoad {
	...
	if(!self.appCredits) {
        NSError *e = nil;
        self.appCredits = [[NSAttributedString alloc] initWithURL:[[NSBundle mainBundle] URLForResource:@"Credits" withExtension:@"rtf"]
        ...
        if (e != noErr) {
            ...
            [self.creditsButton setHidden:YES];
             NSLog(@"PFAboutWindowController did handle exception: %@",e);
        }
	}
	// Set EULA
	if(!self.appEULA) {
        NSError *e;
        self.appEULA = [[NSAttributedString alloc] initWithURL:[[NSBundle mainBundle] URLForResource:@"EULA" withExtension:@"rtf"]
                                                       options:@{NSDocumentTypeDocumentAttribute:NSRTFTextDocumentType}
                                            documentAttributes:nil
                                                         error:&e];
        if (e != noErr) {
